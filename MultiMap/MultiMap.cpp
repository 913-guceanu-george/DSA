...
{ // O(n)
    ...
    this->hashCounter *= RESIZE_MULTIPLICATION_FACTOR; // resize the hash counter

    for (int i = 0; i < this->hashSize * RESIZE_MULTIPLICATION_FACTOR; i++)
    {
        newArray[i] = nullptr; // init the new array
    }
...
bool MultiMap::remove(TKey c, TValue v)
{
    ...
    if (this->table[hashPosition]->key == c)
    {
        ...
        if (this->table[hashPosition]->length >= 2)
        ...
        { // only one element in the array of the given key => delete the node
...
vector<TKey> MultiMap::keySet() const
{
    ...
    for (int i = 0; i <= this->hashSize; i++)
    {
        if (this->table[i] != nullptr)
            keys.push_back(this->table[i]->key);
    }
    return keys;
}
